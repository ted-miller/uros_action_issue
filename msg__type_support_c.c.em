...
@[      elif isinstance(member.type.value_type, NamespacedType)]@
    ...
    if(size > ros_message->@(member.name).capacity){
      fprintf(stderr, "1 cannot allocate received sequence in ros_message @(message.structure.namespaced_type.name) - @(member.name)\n");
...
@[      elif isinstance(member.type.value_type, AbstractString)]@
    ...
    if(size > ros_message->@(member.name).capacity){
      fprintf(stderr, "2 cannot allocate received sequence in ros_message _@(message.structure.namespaced_type.name) - @(member.name)\n");
      fprintf(stderr, "size = %d, capacity = %d\n", size, ros_message->@(member.name).capacity);
      return 0;
    }
    ros_message->@(member.name).size = size;
